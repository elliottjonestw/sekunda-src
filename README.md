...
Connect your iCloud inbox and you get a **Mail page** plus an assistant that can read your email. **iCloud only, nothing stored.** There is no sending and no copy of a message anywhere in this app. From the reader you can do exactly two things that change a mailbox — **mark a message read** (automatically, when you open it) and **move a message to Trash** (a Delete button); everything else is read-only, and **the assistant is read-only entirely** — it never marks or deletes anything.
...
- **Only two writes, and only from the reader** — mark-as-read and move-to-Trash. There is still no send, no reply, no archive, and no move-to-arbitrary-folder, and the *assistant* writes nothing at all: its read-only guarantee is the server's, enforced by `EXAMINE` and `BODY.PEEK`, and holds even against a prompt injection. The two writes were added deliberately for the human at the keyboard — reading state that silently diverged from Apple Mail, and no way to clear an inbox, were the two things people actually missed.
